...
static void serial47xx_enable_ms(struct uart_port *port){
	__set(UART_IER, UARTIER_MIE);    
}

static unsigned int check_modem_status(struct uart_port *port);
static unsigned int serial47xx_get_mctrl(struct uart_port *port)
{
	unsigned int status;
	unsigned int ret;
    
	status = check_modem_status(port);

	ret = 0;
	if (status & UARTMSR_CTS)
		ret |= TIOCM_CTS;
...
{   
	if (mctrl & TIOCM_RTS)
        __set(UART_MCR, UARTMCR_RTS);

    if (!(mctrl & TIOCM_RTS))
        __clr(UART_MCR, UARTMCR_RTS);
}
...
#define DEVICE_CLOCK	cpm_get_clock(CGU_UARTCLK)
...
static unsigned int check_modem_status(struct uart_port *port)
{
	unsigned int status = __rd(UART_MSR);

    if (status & UARTMSR_CCTS)
...
static int serial47xx_clock_enable(int channel, int enabled){
    switch(channel){
    case 0:
        if (enabled)
            cpm_start_clock(CGM_UART0);
        else
	    cpm_stop_clock(CGM_UART0);
        break;
        
    case 1:
        if (enabled)
	    cpm_start_clock(CGM_UART1);
        else
	    cpm_stop_clock(CGM_UART1);
        break;
        
    case 2:
        if (enabled)
	    cpm_start_clock(CGM_UART2);
        else
	    cpm_stop_clock(CGM_UART2);
        break;
        
    case 3:
        if (enabled)
	    cpm_start_clock(CGM_UART3);
        else
	    cpm_stop_clock(CGM_UART3);
        break;
...
static int serial47xx_startup(struct uart_port *port){
    ...
    serial47xx_clock_enable(port->line, 1);
    ...
    __wr(UART_FCR, UARTFCR_FE | UARTFCR_RFLS | UARTFCR_TFLS | UARTFCR_RTRG_15 | UARTFCR_UUE);
...
serial47xx_set_termios(struct uart_port *port, struct ktermios *termios,
    ...
    {
        uprintk("serial ->Mode enabled.\n");
        __set(UART_IER, UARTIER_MIE);
        __set(UART_MCR, UARTMCR_MCE | UARTMCR_FCM);
    }else
    {
        uprintk("serial ->Mode disabled.\n");
        __clr(UART_IER, UARTIER_MIE);
        __set(UART_MCR, 0);        
    }
