...
// for test only
	...
	while (i++ < 10)
		temp_data[i] = 0xAA;
	ep932_reg_write(0x10, temp_data, 10);
	memset(temp_data, 0, 128);
	ep932_reg_read(0x10, temp_data, 10);
	while (i--)
	printk("maxueyue ======================= temp_data = %#x\n", temp_data[i]);
	while(1)
		mdelay(10000);
#if 0
...
void hdmi_tx_power_up(void)
{
	...
	DBG_PRINTK(("EP932_GENERAL_CONTROL_1 0X%02x  0X%02x \r\n",temp_data[0],EP932_GENERAL_CONTROL_1__PU));
}
