/*
 * kernel/drivers/video/panel/jz_AT070TNA2.c -- Ingenic LCD panel device
 *
...
static void AT070TNA2_lvds_config(void)
{
	printk("*************************** LVDS init ****************************\n");
...
static void AT070TNA2_lvds_on(void){
	printk("AT070TNA2_lvds_on\n");
	//AT070TNA2_lvds_config();
	REG_LVDS_TXCTRL = REG_LVDS_TXCTRL | 0x1;
}

static void AT070TNA2_lvds_off(void){
	printk("AT070TNA2_lvds_off\n");
	REG_LVDS_TXCTRL = REG_LVDS_TXCTRL & (~0x1);
}

static void AT070TNA2_panel_display_init(void)
{
	AT070TNA2_lvds_config();
}

static void AT070TNA2_panel_display_on(void)
{
	if(!lcd_board)
...
static void AT070TNA2_panel_display_off(void)
{
	if(!lcd_board)
		printk("%s: lcd_board is null\n", __func__);
...
static int AT070TNA2_panel_probe(struct lcd_soc_info *soc , struct lcd_board_info *board)
{
...
static struct lcd_panel_ops_info AT070TNA2_panel_ops = {
	.panel_init		=	AT070TNA2_panel_display_init,
	.panel_display_on	=	AT070TNA2_panel_display_on,
	.panel_display_off	=	AT070TNA2_panel_display_off,
	.panel_probe		=	AT070TNA2_panel_probe,
};

static struct lcd_panel_attr_info AT070TNA2_panel_attr = {
	.bpp = 24, /* */
...
static struct lcd_panel_info AT070TNA2_lcd_panel = {
	.name = "AT070TNA2",
	.panel_probable = 0,	// not probable
	.panel_attr = &AT070TNA2_panel_attr,
	.panel_ops = &AT070TNA2_panel_ops,
};

static int __init AT070TNA2_init(void)
{
	// register the panel with lcd drivers
	lcd_register_panel(&AT070TNA2_lcd_panel);
	return 0;
}

module_init(AT070TNA2_init);
