...
#endif 
static bool  Ctp_it7260_quit_update_mode(void )
{
	  //it7260/7260 启动时会检测irq脚，如果超过一定时间是高电平，会进入烧录模式，所以此处先拉低一会
	  
	__gpio_as_output(ctp_it7260_pdata->power);
	__gpio_clear_pin(ctp_it7260_pdata->power);
	
 	__gpio_as_output(ctp_it7260_pdata->intr);
 	__gpio_clear_pin(ctp_it7260_pdata->intr);	
	msleep(20);
	__gpio_set_pin(ctp_it7260_pdata->power);
	msleep(20);
	
	//ExitFirmwareUpgradeMode(client);
	__gpio_set_pin(ctp_it7260_pdata->intr);
	__gpio_as_input(ctp_it7260_pdata->intr);
	return true;
}
...
static bool Ctp_it7260_init_irq(struct i2c_client *client)
{
	...
	Ctp_it7260 = i2c_get_clientdata(client);
	 ...
	 Ctp_it7260_quit_update_mode();
	__gpio_as_irq_low_level(ctp_it7260_pdata->intr);
	__gpio_disable_pull(ctp_it7260_pdata->intr);
	
	ret = request_irq(client->irq, Ctp_it7260_touch_irq, IRQF_DISABLED, "ctp_it7260_irq", Ctp_it7260);
	if (ret ) {
...
static void  Ctp_it7260_work_func(struct work_struct  *work)
{
	...
	int xx= 0;
	int yy= 0;
	PE1status =  __gpio_get_pin(ctp_it7260_pdata->intr);
	if (!PE1status)
	{
