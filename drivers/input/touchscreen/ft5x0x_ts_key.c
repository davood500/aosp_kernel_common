...
#define PENUP_TIMEOUT_TIMER 1
#define P2_PACKET_LEN 11  //1 point
#define P1_PACKET_LEN 5
#endif
...
static void ft5x0x_ts_power_on(void)
{
#ifdef GPIO_FT5X0X_EN
	__gpio_as_output(GPIO_FT5X0X_EN);	
	__gpio_set_pin(GPIO_FT5X0X_EN);	
#endif
}

static void ft5x0x_ts_power_off(void)
{
#ifdef GPIO_FT5X0X_EN
	__gpio_as_output(GPIO_FT5X0X_EN);	
	__gpio_clear_pin(GPIO_FT5X0X_EN);	
#endif
}
...
static void ft5x0x_chip_reset(void)
{
	__gpio_as_output(pdata->wake);
	__gpio_set_pin(pdata->wake);
	__gpio_clear_pin(pdata->wake);
	msleep(20);
	__gpio_set_pin(pdata->wake);

	__gpio_as_irq_low_level(pdata->intr);
	__gpio_disable_pull(pdata->intr);
}

#ifdef CONFIG_FT5406_SWAP_XY
...
static int ft5x0x_read_data(void)
{
	...
	buf[0] = 2;
#ifdef CONFIG_FT5X0X_MULTITOUCH
	ret = ft5x0x_i2c_rxdata(buf, P2_PACKET_LEN);
#else
	ret = ft5x0x_i2c_rxdata(buf, P1_PACKET_LEN);
#endif
	if (ret < 0) {
		ft5x0x_chip_reset();
...
static void ft5x0x_report_value(void)
{
	...
	event->x1 = (event->x1*pdata->x_res)/800;
	event->y1 = (event->y1*pdata->y_res)/480;
	event->x2 = (event->x1*pdata->x_res)/800;
	event->y2 = (event->y1*pdata->y_res)/480;
#endif
...
//end
	...
	input_set_abs_params(input_dev,
			ABS_MT_POSITION_X, 0, pdata->x_res, 0, 0);
	input_set_abs_params(input_dev,
			ABS_MT_POSITION_Y, 0, pdata->y_res, 0, 0);
	input_set_abs_params(input_dev,
			ABS_MT_TOUCH_MAJOR, 0, pdata->pressure_max, 0, 0);
	input_set_abs_params(input_dev,
	...
	input_set_abs_params(input_dev,
			ABS_PRESSURE, 0, pdata->pressure_max, 0 , 0);
#endif
