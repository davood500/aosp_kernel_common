...
static inline unsigned int jz_mmc_get_gap(struct jz_mmc_host *host)
{
	if((host->mmc->card->state & (0b1000)))              //SDHC card
		return (SBUF / 512);
	else
		return SBUF;
}

static unsigned char jz_mmc_get_state(struct jz_mmc_host *host, struct mmc_command *cmd)
{
	enum trans_state state;
	if (host->mmc->card && (host->mmc->card->type < 2)){
		if(cmd->opcode == MMC_READ_MULTIPLE_BLOCK) {
			host->buf_gap = jz_mmc_get_gap(host);
			if (cmd->arg + host->buf_gap * 2 > host->mmc->capacity) {
				state = MSC_CARD_NROP;
				return state;
			}

			if(sg_dma_len(host->curr.data->sg) == SBUF)
				if((cmd->arg - host->arg_last == host->buf_gap) 
				   && ((host->trans_state_last == 3) || (host->trans_state_last == 4)) 
				   && (cmd->arg > host->arg_last))					
					
					state = MSC_CARD_ALAL;
				else
					state = MSC_CARD_MAL;
			else if((cmd->arg - host->arg_last == host->buf_gap)
				&& ((host->trans_state_last == 3) || (host->trans_state_last == 4)) 
				&& (cmd->arg > host->arg_last))

				state = MSC_CARD_ALNAL;
			else
				state = MSC_CARD_NAL;
		} else
			state = MSC_CARD_NROP;
...
void jz_mmc_data_start(struct jz_mmc_host *host)
{
	...
	if(host->trans_state == MSC_CARD_MAL)
		REG_MSC_NOB(host->pdev_id) = 256;                 //256 for 64k buffer,128 for 32k buffer
#endif
...
#ifdef CONFIG_PARALLEL_DMA_MODE1
static int jz_mmc_qread_done(struct jz_mmc_host *host)
{
	if(host->eject) {
		printk("PARALLEL_DMA:data trans eject----After copy\n");
		jz_mmc_handle_exception(host);
		host->dma_flag = 1;
		return -1;
	}
	if (host->curr.data->error == 0)
		host->curr.data->bytes_xfered = host->curr.data->blocks * host->curr.data->blksz;
	else
		host->curr.data->bytes_xfered = 0;
...
#ifdef CONFIG_PARALLEL_DMA_MODE1
	if (host->mmc->card && (host->mmc->card->type < 2)){
		wait_event_interruptible_timeout(host->parallel_queue,host->dma_flag || (host->eject),5 * HZ);
		if(host->eject) {
			printk("PARALLEL_DMA:data trans eject----Waiting DMA RX\n");
			jz_mmc_handle_exception(host);
			host->dma_flag = 1;
			return;
		} else if (host->dma_flag)
			host->trans_state_last = host->trans_state;
		else {
			printk("PARALLEL_DMA:Time out while waiting parallel_queue\n");
			jz_mmc_handle_exception(host);
			host->dma_flag = 1;
			return;
		}
	}
	host->trans_state = jz_mmc_get_state(host,cmd);
#ifdef PARALLEL_DBG
	printk("===id: %d   state: %d   opcode = %d   arg = %X",host->pdev_id, host->trans_state,cmd->opcode,cmd->arg);
	if(cmd->opcode == 17 || cmd->opcode == 18) {
		printk("  trans_state_last = %d   arg_last = %X   sg_len = %d\n"
		       ,host->trans_state_last,host->arg_last,sg_dma_len(host->curr.data->sg));
	} else {
		printk("\n");
	}
#endif
	if(host->trans_state >= MSC_CARD_ALAL) {
		host->arg_last = cmd->arg;
		host->cmd_bak = cmd->opcode;
		host->dma_flag = 0;
		if(host->devp)
			up(&host->devp->sem);	
		memcpy(sg_virt(host->curr.data->sg),host->dma_stream_buf + SBUF / 4,sg_dma_len(host->curr.data->sg));
		
		if(jz_mmc_qread_done(host) < 0)
			return;

		if(host->trans_state == MSC_CARD_ALAL) {
			if(jz_mmc_quick_read(host) < 0)				
				return;
		} else
			host->dma_flag = 1;
		jz_mmc_clean_curr(host);
		return;
	} else if((host->trans_state > MSC_CARD_NROP) && (cmd->opcode == MMC_READ_SINGLE_BLOCK || cmd->opcode == MMC_READ_MULTIPLE_BLOCK))
		host->arg_last = cmd->arg;
	else
		host->arg_last = 0;
#endif
...
#ifdef CONFIG_PARALLEL_DMA_MODE1
	if(host->trans_state == MSC_CARD_MAL) {
		host->cmdat_bak = cmdat | MSC_CMDAT_RTRG_EQUALT_16 | MSC_CMDAT_TTRG_LESS_16 | MSC_CMDAT_SEND_AS_STOP;
		host->cmd_bak = cmd->opcode;
	}
#endif
	...
	if (host->curr.data) {
		...
		if(host->curr.data->flags & MMC_DATA_WRITE) {
			...
			if(host->dma_ack || host->dma_start_addr == 0) {
				...
				if(host->trans_state == MSC_CARD_MAL) {
					host->dma_flag = 1;
					memcpy(sg_virt(host->curr.data->sg),host->dma_stream_buf,SBUF);
				}
#endif
				jz_mmc_data_done(host);
...
static int jz_mmc_data_done(struct jz_mmc_host *host)
{
	...
	if(host->cpu_trans_bytes) {
		...
		if(host->flag_cp)
			memcpy((void *)sg_virt(data->sg), host->dma_buf, host->aligned_bytes);
...
#ifdef CONFIG_PARALLEL_DMA_MODE1
		init_waitqueue_head(&host->parallel_queue);
		host->dma_flag = 1;
#endif
