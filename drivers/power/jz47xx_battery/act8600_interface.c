...
int get_state(struct act8600_interface *interface)
{
	...
	interface->ad = ((intr1 & ACT8600_APCH_INTR1_INDAT) != 0);
	interface->usb = (((otg_con & ACT8600_OTG_CON_VBUSDAT) != 0) && ((otg_con & ACT8600_OTG_CON_Q1) == 0));

#if CONFIG_ACTBAT_DC
	if(intr1 & ACT8600_APCH_INTR1_INDAT) {
	...
	switch(chgst & ACT8600_APCH_STAT_STATE_MASK)
	{
		...
		case ACT8600_APCH_STAT_STATE_SUSPEND:
			if(((otg_con & ACT8600_OTG_CON_VBUSDAT) != 0) && ((otg_con & ACT8600_OTG_CON_Q1) == 0)) {
				act8600_set_q3(1);
				return POWER_SUPPLY_STATUS_CHARGING;
			}
