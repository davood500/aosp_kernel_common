...
#define DEBUG_READ_WRITE_REGISTER 0

#if DEBUG_READ_WRITE_REGISTER
static int gRegRCnt=0;
static int gRegWCnt=0;
#endif
...
// 	pgprot_val(mdlMap->vma->vm_page_prot) |= _CACHE_UNCACHED;		/* Uncacheable */
...
#ifdef ANDROID
	MDBG("dma_sync_single_for_device");
...
{
//	printk("gckOS_CacheFlush()  current->pid=%4d current->tgid=%4d Os=0x%8x, Process=%4d, Logical=%p, Bytes=%d\n", 
//	       current->pid, current->tgid, (int)Os, (int)Process, Logical, (int)Bytes);
#if USE_JZ_FLUSH_CACHE
