...
#endif
#ifndef GPU_MEM_BASE
#define GPU_MEM_BASE 0		/* if GPU_MEM_BASE = 0, alloc gpu memory dynamicly on bootup */
#endif
#ifndef GPU_MEM_SIZE
#define GPU_MEM_SIZE 0x400000	/* set default reserved memory 4M Bytes. */
#endif
...
long contiguousSize = GPU_MEM_SIZE;
module_param(contiguousSize, long, 0644);

ulong contiguousBase = GPU_MEM_BASE;
...
static int __devinit gpu_probe(struct platform_device *pdev)
{
	...
	irqLine = res->start;
	res = platform_get_resource_byname(pdev, IORESOURCE_IO,"gpu_base");
	if (!res) {
