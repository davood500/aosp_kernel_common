...
static struct linux_sensor_t ori_hardware_data = {                                                                           
	                  "fake orientation base g-sensor",
	                   "fake jz sensor",
	       SENSOR_TYPE_ORIENTATION,0,64*90,90, 1, { } 
};
...
//	printk("orientation report:x[%d],y[%d],pitch[%d],roll[%d] \n",x,y,pitch,roll);
}
...
static int __init orientation_init(void)
{
	...
	input_set_abs_params(input_dev, ABS_RX, -8000, 8000, 64, 0);
	input_set_abs_params(input_dev, ABS_RY, -8000, 8000, 64, 0);
	input_set_abs_params(input_dev, ABS_RZ, -8000, 8000, 64, 0);
...
MODULE_DESCRIPTION("fake orientation driver based on lis35de");
MODULE_AUTHOR("xpdong@ingenic.cn");
MODULE_LICENSE("GPL");
