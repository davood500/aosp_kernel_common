...
static struct {
	...
	{
	3,	 LSM303DLH_ACC_ODR1000}, {
	10,	 LSM303DLH_ACC_ODR400}, {
	20,	 LSM303DLH_ACC_ODR100}, {
	100,	 LSM303DLH_ACC_ODR50}, {
	200,	 LSM303DLH_ACC_ODR10}, {
	500,	 LSM303DLH_ACC_ODR5}, {
	1000,	 LSM303DLH_ACC_ODR2}, {
	2000,	 LSM303DLH_ACC_ODR1}, {
	0,	 LSM303DLH_ACC_ODRHALF},};
...
struct linux_sensor_t hardware_data = {                                                                           
	                  "lsm303dlh_acc 3-axis Accelerometer",
	                   "ST sensor",
	       SENSOR_TYPE_ACCELEROMETER,0,64,1, 1, { } // 类型是重力感应硬件,重力感应的数值是1个G，上传的数据向量最大为64，估计电流为4mA.
};
...
#ifdef CONFIG_SENSORS_LSM303DLH_ORI
extern void orientation_report_values(int *xyz);
#endif
				  ...
				  u8 *buf, int len)
{
	...
	if (err < 0){
		printk("acc read i2c_transfer is error\n");
		return err;
	}
	return 0;
}
				   ...
				   u8 *buf, int len)
{
	...
	if (err < 0){
		printk("acc write i2c_transfer is error\n");
		return err;
	}
...
static void lsm303dlh_acc_device_power_off(struct lsm303dlh_acc_data *acc)
{
	...
	lsm303dlh_acc_i2c_read(acc, acc_data, 1);
	buf[1]= acc_data[0];
					       ...
					       int *xyz)
{
	...
	xyz[0] = ((acc->pdata->negate_x) ? (-hw_d[acc->pdata->axis_map_x])
		  : (hw_d[acc->pdata->axis_map_x])) >> 4;
	xyz[1] = ((acc->pdata->negate_y) ? (-hw_d[acc->pdata->axis_map_y])
		  : (hw_d[acc->pdata->axis_map_y])) >> 4;
	xyz[2] = ((acc->pdata->negate_z) ? (-hw_d[acc->pdata->axis_map_z])
		  : (hw_d[acc->pdata->axis_map_z])) >> 4;
...
long lsm303dlh_acc_misc_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SENSOR_IOCTL_SET_DELAY:
		...
		if (interval < acc->pdata->min_interval )
			interval = acc->pdata->min_interval;
		else if (interval > acc->pdata->max_interval)
			interval = acc->pdata->max_interval;
		acc->pdata->poll_interval = interval;
		err = lsm303dlh_acc_update_odr(acc, acc->pdata->poll_interval);
		/* TODiO: if update fails poll is still set */
		if (err < 0)
			return err;
		break;
	...
	case SENSOR_IOCTL_GET_DATA_MAXRANGE:
		if (copy_to_user(argp, &acc->pdata->g_range, sizeof(acc->pdata->g_range)))
			return -EFAULT;
		break;

	case SENSOR_IOCTL_WAKE:
		input_event(acc->input_dev, EV_SYN,SYN_CONFIG, 0);
		break;
	default:
		return -EINVAL;
	}
...
static const struct file_operations lsm303dlh_acc_misc_fops = {
	...
	.unlocked_ioctl = lsm303dlh_acc_misc_ioctl,
};
...
#ifdef CONFIG_LSM303DLH_ACC_AS_INPUT
static int lsm303dlh_acc_input_init(struct lsm303dlh_acc_data *acc)
{
	...
	acc->input_dev->name = G_SENSOR_NAME;
