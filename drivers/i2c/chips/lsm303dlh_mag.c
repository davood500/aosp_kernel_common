...
static struct workqueue_struct *mag_wqueue;
...
struct linux_sensor_t mag_hardware_data = {
		"lsm303dlh_mag 3-axis Magnetic",
		"ST motion sensor",
		SENSOR_TYPE_MAGNETIC_FIELD,0,1,64,1 ,{}
};
				  ...
				  u8 *buf, int len)
{
	...
	if (err < 0){
		printk("mag read i2c_transfer is error\n");
		return err;
	}
	return 0;
}
				   ...
				   u8 *buf, int len)
{
	...
	if (err < 0){
		printk("mag write i2c_transfer is error\n");
		return err;
	}
	return 0;
}
...
static int lsm303dlh_mag_hw_init(struct lsm303dlh_mag_data *mag)
{
	...
	err = lsm303dlh_mag_i2c_write(mag, buf, 4);
...
static int lsm303dlh_mag_disable(struct lsm303dlh_mag_data *mag)
{
	if (atomic_cmpxchg(&mag->enabled, 1, 0)) {
		cancel_work_sync(&mag->input_work);
		lsm303dlh_mag_device_power_off(mag);
	}
...
static long lsm303dlh_mag_misc_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SENSOR_IOCTL_GET_DATA:
		if (copy_to_user(argp, &mag_hardware_data, sizeof(mag_hardware_data)))
			return -EINVAL;
		break;
	case SENSOR_IOCTL_GET_DATA_MAXRANGE:
		if (copy_to_user(argp, &mag->pdata->h_range, sizeof(mag->pdata->h_range)))
			return -EFAULT;
...
static void lsm303dlh_mag_input_work_func(struct work_struct *work)
{
	...
	mag  = container_of((struct work_struct *)work,
			     struct lsm303dlh_mag_data, input_work);

	mutex_lock(&mag->lock);
	buf[0]=SR_REG_M;
	
	err = lsm303dlh_mag_i2c_read(mag, buf, 2);
	if (err < 0)
		dev_err(&mag->client->dev, "mag i2c read failed\n");
	...
	enable_irq(mag->client->irq);
	mutex_unlock(&mag->lock);
}

#ifdef CONFIG_LSM303DLH_MAG_AS_INPUT
...
static irqreturn_t lsm303dlh_mag_interrupt(int irq, void *dev_id)
{
	struct lsm303dlh_mag_data *mag = dev_id;
	disable_irq_nosync(mag->client->irq);
	__gpio_ack_irq(mag->pdata->intr);
	if (!work_pending(&mag->input_work)) {
		queue_work(mag_wqueue, &mag->input_work);
	}
	return IRQ_HANDLED;
}
static int lsm303dlh_mag_input_init(struct lsm303dlh_mag_data *mag)
{
	...
	INIT_WORK(&mag->input_work, lsm303dlh_mag_input_work_func);
//-------------
	mag_wqueue = create_singlethread_workqueue(NAME);
	if(!mag_wqueue){
		printk("lsm mag creata workqueue is failed\n");
		return -1;
	}
	err = request_irq(mag->client->irq, lsm303dlh_mag_interrupt, IRQF_DISABLED,
			NAME,mag);
	if (err < 0) {
		printk(" request irq is error\n");
		dev_err(&mag->client->dev, "%s rquest irq failed\n", __func__);
		return err;
	}
	__gpio_as_irq_rise_edge(mag->pdata->intr);
//-----------------
...
#ifdef LSM303DLH_MAG_OPEN_ENABLE
	...
	mag->input_dev->name = MAGNETIC_SENSOR_NAME;
...
/******************************************/
	//don't use mag after, so power off !!!!!!
	lsm303dlh_mag_device_power_off(mag);
	return 0;
/******************************************/
	if (mag->pdata->init) {
