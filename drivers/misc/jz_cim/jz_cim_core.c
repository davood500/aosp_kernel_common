...
#else
static DEFINE_SPINLOCK( fresh_lock );
#endif

static int fresh_id __read_mostly;
...
void cim_scan_sensor(void)
{
	...
	static struct camera_sensor_desc *tmp_desc = NULL;
	...
	list_for_each_entry(si, &sensor_list, list)
	{
		...
		if(tmp_desc != NULL)
			tmp_desc->ops->sensor_set_power(1);
...
void sensors_make_default(void)
{
	...
	list_for_each_entry(si, &sensor_list, list)
	{
		if(si->facing == CAMERA_FACING_BACK) {
			si->sensor_id = jz_cim->ginfo.sensor_count;
			si->ops->sensor_set_power(1);
			jz_cim->ginfo.sensor_count++;
			printk("sensor_name:%s\t\tid:%d facing:%d\n",si->name,si->sensor_id,si->facing);
			cur_desc = si;
		}
	}
	...
	list_for_each_entry(si, &sensor_list, list)
	{
		if(si->facing == CAMERA_FACING_FRONT) {
			si->sensor_id = jz_cim->ginfo.sensor_count;
			si->ops->sensor_set_power(1);
			jz_cim->ginfo.sensor_count++;
			printk("sensor_name:%s\t\tid:%d facing:%d\n",si->name,si->sensor_id,si->facing);
...
//--------------------------------------------------------------------------------------------
	...
	for (i= 0; i< SWAP_NR; i++)
	{
		frame_desc[i].framebuf = jz_cim->preview_binfo.base_paddr + preview_frmsize * i;
...
static void cim_init_config(struct camera_sensor_desc *desc)
{
	...
	jz_cim->cim_cfg.cfg = cur_desc->cfg_info.configure_register & ~(CIM_CFG_PACK_MASK);
#if defined(CONFIG_SOC_JZ4770)
	switch(cur_desc->cfg_info.format) {
		case SENSOR_OUTPUT_FORMAT_Y0UY1V:jz_cim->cim_cfg.cfg |= CIM_CFG_PACK_4;break;
		case SENSOR_OUTPUT_FORMAT_UY1VY0:jz_cim->cim_cfg.cfg |= CIM_CFG_PACK_5;break;
		case SENSOR_OUTPUT_FORMAT_Y1VY0U:jz_cim->cim_cfg.cfg |= CIM_CFG_PACK_6;break;
		case SENSOR_OUTPUT_FORMAT_VY0UY1:jz_cim->cim_cfg.cfg |= CIM_CFG_PACK_7;break;
		case SENSOR_OUTPUT_FORMAT_VY1UY0:jz_cim->cim_cfg.cfg |= CIM_CFG_PACK_0;break;
		case SENSOR_OUTPUT_FORMAT_Y1UY0V:jz_cim->cim_cfg.cfg |= CIM_CFG_PACK_3;break;
		case SENSOR_OUTPUT_FORMAT_UY0VY1:jz_cim->cim_cfg.cfg |= CIM_CFG_PACK_2;break;
		case SENSOR_OUTPUT_FORMAT_Y0VY1U:jz_cim->cim_cfg.cfg |= CIM_CFG_PACK_1;break;		
		//4770 request cim  output  Y0UY1V
		default :
			printk("Error!! cur_desc->cfg_info.format value is wrong!!!!! \n");
			break;
	}
#elif defined(CONFIG_SOC_JZ4760) || defined(CONFIG_SOC_JZ4760B)
	switch(cur_desc->cfg_info.format) {
		case SENSOR_OUTPUT_FORMAT_Y0UY1V:jz_cim->cim_cfg.cfg |= CIM_CFG_PACK_2;break;
		case SENSOR_OUTPUT_FORMAT_UY1VY0:jz_cim->cim_cfg.cfg |= CIM_CFG_PACK_1;break;
		case SENSOR_OUTPUT_FORMAT_Y1VY0U:jz_cim->cim_cfg.cfg |= CIM_CFG_PACK_0;break;
		case SENSOR_OUTPUT_FORMAT_VY0UY1:jz_cim->cim_cfg.cfg |= CIM_CFG_PACK_3;break;
		case SENSOR_OUTPUT_FORMAT_VY1UY0:jz_cim->cim_cfg.cfg |= CIM_CFG_PACK_6;break;
		case SENSOR_OUTPUT_FORMAT_Y1UY0V:jz_cim->cim_cfg.cfg |= CIM_CFG_PACK_7;break;
		case SENSOR_OUTPUT_FORMAT_UY0VY1:jz_cim->cim_cfg.cfg |= CIM_CFG_PACK_4;break;
		case SENSOR_OUTPUT_FORMAT_Y0VY1U:jz_cim->cim_cfg.cfg |= CIM_CFG_PACK_5;break;		
		//4760 request cim  output  UY0VY1
		default :
			printk("Error!! cur_desc->cfg_info.format value is wrong!!!!! \n");
			break;
	}
	...
	jz_cim->cim_cfg.cfg |= CIM_CFG_DMA_BURST_INCR32 | (3<<CIM_CFG_RXF_TRIG_BIT);//(n+1)*burst = 4*32 = 128 < 256
...
static void cim_stop_snapshot(void)
{
	__cim_disable();
	__cim_reset_rxfifo();
	...
	if(waitqueue_active(&jz_cim->capture_wait_queue))
...
static int cim_snapshot(void)
{
	...
	else
	{
		...
		if(!interruptible_sleep_on_timeout(&jz_cim->capture_wait_queue,
...
{  
	...
	dprintk("current task(%d)'s pgd is 0x%p\n",current->pid,current->mm->pgd);
...
static unsigned int cim_get_preview_buf(int is_pbuf)
{
	...
	while(fresh_id == -1) {
		...
		if(!interruptible_sleep_on_timeout(&jz_cim->preview_wait_queue,msecs_to_jiffies(300)))
			return (unsigned int)(~0);
		if(jz_cim->cim_transfer_started == 0)
			return (unsigned int)(~0);
	}
...
static int cim_get_sensor_info(void __user *arg,struct sensor_info *info)
{
	...
	list_for_each_entry(si, &sensor_list, list)
	{
		if(si->sensor_id == info->sensor_id) break;

	}

	mutex_unlock(&sensor_lock);

	fill_info_by_desc(info,si);
...
static int cim_set_preview_resolution(struct resolution_info param)
{
	...
	jz_cim->preview_binfo.buffer_size = framesize * SWAP_NR;
...
static int cim_set_capture_resoultion(struct resolution_info param,int state)
{
	...
	jz_cim->capture_binfo.buffer_size = framesize;
...
//---------------------------------------------------------------------------------------------
	...
	dprintk("SET CAPTURE RESOULTION %d\tX\t%d\t%d -- OK!\n",param.width,param.height,param.bpp);
	return 0;
}
...
#endif
{
	...
	if(cmd == IOCTL_CIM_STOP_PREVIEW)
	{
		dprintk("IOCTL_CIM_STOP_PREVIEW\n");
		cim_stop_preview();
		return 0;
	} else if(cmd == IOCTL_CIM_STOP_SNAPSHOT) {
	...
	{
		retval = cim_select_sensor(arg);
		goto cim_ioctl_end;
	}
	case IOCTL_CIM_GET_PREVIEW_PARAM:
	{
		retval = copy_to_user(argp, &cur_desc->preview_parm, sizeof(struct resolution_info)) ? -EFAULT : 0;
		goto cim_ioctl_end;
	}
	...
	{
		retval = copy_to_user(argp, &cur_desc->capture_parm, sizeof(struct resolution_info)) ? -EFAULT : 0;
		goto cim_ioctl_end;
	}
	case IOCTL_CIM_GET_CAPTURE_BUF_INFO:
	{	
		retval = copy_to_user(argp, &jz_cim->capture_binfo, sizeof(struct camera_buffer_info)) ? -EFAULT : 0;
		goto cim_ioctl_end;
	}
	case IOCTL_CIM_GET_PREVIEW_BUF_INFO:
	{
		retval = copy_to_user(argp, &jz_cim->preview_binfo, sizeof(struct camera_buffer_info)) ? -EFAULT : 0;
		goto cim_ioctl_end;
	}
	case IOCTL_CIM_SET_PREVIEW_MEM:
	{
		dprintk("IOCTL_SET_PREVIEW_MEM\n");
		retval = cim_set_buffer(argp,&jz_cim->preview_binfo);
		goto cim_ioctl_end;
	}
	case IOCTL_CIM_SET_CAPTURE_MEM:
	{
		dprintk("IOCTL_SET_CAPTURE_MEM\n");
		retval = cim_set_buffer(argp,&jz_cim->capture_binfo);
		goto cim_ioctl_end;
	}
	case IOCTL_CIM_GET_PREVIEW_OFFSET:
	{
		...
		retval = copy_to_user(argp, &offset, sizeof(offset)) ? -EFAULT : 0;
		goto cim_ioctl_end;
	}
	...
	case IOCTL_CIM_GET_GLOBAL_INFO:
	{
		...
		retval = cim_get_global_info(argp);
		goto cim_ioctl_end;
	}
	case IOCTL_CIM_GET_SUPPORT_SIZE:
	{
		dprintk("IOCTL_GET_SUPPORT_SIZE\n");
		retval = cim_get_support_size(argp);
		goto cim_ioctl_end;
	}
	case IOCTL_CIM_GET_SENSOR_INFO:
	{
		...
		retval = cim_get_sensor_info(argp,&info); 
		goto cim_ioctl_end;
	}
	case IOCTL_CIM_SET_PARAM:
	{
		...
		retval = cim_set_param(&param);
		goto cim_ioctl_end;
	}
	case IOCTL_CIM_STOP_FOCUS:
	{
		retval = cim_set_function(5,NULL);
		goto cim_ioctl_end;
	}
	case IOCTL_CIM_DO_FOCUS:
	{
		retval = cim_set_function(3,NULL);
		goto cim_ioctl_end;
	}
	default:
		dprintk("Not supported command: 0x%x\n", cmd);
...
/*static void cim_early_suspend(struct early_suspend *handler)
{
	if(jz_cim->cim_started == TRUE);
	{
		cim_stop();
...
static void cim_early_resume(struct early_suspend *handler)
{
	dprintk("resume\n");
}*/
#endif
...
#if 1 
	dprintk("vma->vm_start: 0x%lx\n",vma->vm_start);
	dprintk("vma->vm_end: 0x%lx\n",vma->vm_end);
	dprintk("vma->vm_pgoff: 0x%lx\n",vma->vm_pgoff);
	dprintk("jz_cim->mem_base(V): 0x%p\n",jz_cim->mem_base);
	dprintk("jz_cim->mem_base(P): 0x%lx\n",virt_to_phys(jz_cim->mem_base));
	dprintk("jz_cim->mem_size: 0x%x\n",jz_cim->mem_size);
...
static int __devinit cim_probe(struct platform_device *dev)
{
	int ret;

	struct jz_cim_platform_data *pdata = dev->dev.platform_data;
	...
	jz_cim->power_on = pdata->power_on;
	jz_cim->power_off = pdata->power_off;
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	/*jz_cim->early_suspend.suspend = cim_early_suspend;
	jz_cim->early_suspend.resume = cim_early_resume;
	jz_cim->early_suspend.level = EARLY_SUSPEND_LEVEL_DISABLE_FB;
	register_early_suspend(&jz_cim->early_suspend);*/
#endif
	printk("Virtual Driver of JZ CIM registered\n");
	return 0;
}

static int __devexit cim_remove(struct platform_device *dev)
{
...
MODULE_DESCRIPTION("SOC Jz47xx Camera Module Interface Driver");
