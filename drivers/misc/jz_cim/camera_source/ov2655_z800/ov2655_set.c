...
void ov2655_capture_reg_set(struct i2c_client *client)
{
...
void ov2655_capture_set(struct i2c_client *client)
{
	...
	Extra_lines = (Reg0x302d<<8) + Reg0x302e;
...
*/
	...
	Capture_Banding_Filter = Capture_PCLK_Frequency*1000000/ 100/(2*Capture_Line_Width) + (5/10);
	...
	else 
	{
		if (Gain_Exposure > Capture_Maximum_Shutter * 16) //16720
		{
			...
			if (Capture_Gain16 > Capture_Max_Gain16) 
			{
				...
				// Exposure = n/100
                                Capture_Exposure = Gain_Exposure*110/100/Capture_Max_Gain16;
	...
	else
	{
		...
		Extra_lines = Capture_Exposure - Shutter;
	}

	ov2655_capture_reg_set(client);
	...
	Reg0x302d = (Extra_lines >>8) & 0x00ff;
	...
	if (Capture_Gain16 > 31) {
        ...
        sensor_write_reg16(client,0x3000, Gain+5);
	Reg0x3000 = sensor_read_reg16(client,0x3000);
        mdelay(100);
...
void ov2655_size_1600x1200(struct i2c_client *client,int mode)
{
...
void ov2655_size_1280x1024(struct i2c_client *client,int mode)
{
	if(mode == CAMERA_MODE_CAPTURE)
	{
		sensor_write_reg16(client,0x3302,0x11);
...
void ov2655_size_1024x768(struct i2c_client *client,int mode)
{
	if(mode == CAMERA_MODE_CAPTURE)
	{
		sensor_write_reg16(client,0x3011,0x01);
...
void ov2655_size_800x600(struct i2c_client *client,int mode)
{
...
void ov2655_size_640x480(struct i2c_client *client,int mode)
{
	if(mode == CAMERA_MODE_PREVIEW)
	{
		sensor_write_reg16(client,0x3302,0x11);
...
void ov2655_size_352x288(struct i2c_client *client,int mode)
{
	if(mode == CAMERA_MODE_PREVIEW)
	{
		sensor_write_reg16(client,0x3302,0x11);
...
void ov2655_size_176x144(struct i2c_client *client,int mode)
{
	if(mode == CAMERA_MODE_PREVIEW)
	{
		sensor_write_reg16(client,0x3302,0x11);
...
void ov2655_size_switch(struct i2c_client *client,int width,int height,int setmode)
{
  //	dprintk("%dx%d - mode(%d)",width,height,setmode);
	...
	{
		ov2655_size_1600x1200(client,setmode);
	}
	else if(width == 1280 && height == 1024)
	{
		ov2655_size_1280x1024(client,setmode);
	}  
	else if(width == 1024 && height == 768)
	{
		ov2655_size_1024x768(client,setmode);
	}
	else if(width == 800 && height == 600)
	{
		ov2655_size_800x600(client,setmode);
	}
	else if(width == 640 && height == 480)
	{
		ov2655_size_640x480(client,setmode);
	} 
	else if(width == 352 && height == 288)
	{
		ov2655_size_352x288(client,setmode);
	}  
	else if(width == 176 && height == 144)
	{
		ov2655_size_176x144(client,setmode);
	}  
	else
		return;
//	mdelay(500);
