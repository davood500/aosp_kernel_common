...
void set_size_352x288(struct i2c_client *client,int mode)
{
	...
	voffset = (((480 - 288) / 2) >> 1) << 1;
	hoffset = (((640 - 352) / 2) >> 1) << 1; 
...
void siv120b_init_setting(struct i2c_client *client)
{
	...
	//[init]
	sensor_write_reg(client,0x00,0x00); //
	sensor_write_reg(client,0x04,0x00); //	#mirror image  modify by fuchao ,if display mirror ,we will modify sensor ori or add compile config here 
	...
	//denoise
	sensor_write_reg(client,0x60,0x2f); //0x41                     
	sensor_write_reg(client,0x62,0xb7); //0xb8                    
	sensor_write_reg(client,0x63,0xb7); //0x07                    
	sensor_write_reg(client,0x64,0xb7); //0x14                     
	sensor_write_reg(client,0x65,0x0c); //0x1e                     
	sensor_write_reg(client,0x66,0xff); //0x41                     
	sensor_write_reg(client,0x67,0x08); //0xb8                    
	sensor_write_reg(client,0x68,0x00); //0x07                    
	sensor_write_reg(client,0x69,0x00); //0x14                     
	sensor_write_reg(client,0x6a,0x00); //0x1e                     
	sensor_write_reg(client,0x6b,0x00); //0x1e                     
	sensor_write_reg(client,0x6c,0xa0); //0x1e                     
	sensor_write_reg(client,0x6d,0x10); //0x1e                     
	sensor_write_reg(client,0x6e,0x08); //0x1e                     
	sensor_write_reg(client,0x6f,0x18); //0x1e                     
