...
static balance_flag_t cur_wb = WHITE_BALANCE_AUTO;
int siv120b_set_balance(balance_flag_t balance_flag,int arg)
{
	dprintk("siv120b_set_balance");
	if(cur_wb == balance_flag)
		return 0;//avoid sensor bug when setting wb auto appears blue shadow
	switch(balance_flag)
	{
...
static ssize_t siv120b_read_store(struct device *dev,
...
static ssize_t siv120b_write_store(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t count)
{
	uint32_t reg,val;
...
static DEVICE_ATTR(write, 0664, NULL, siv120b_write_store);
static DEVICE_ATTR(read, 0664, NULL, siv120b_read_store);

static struct attribute *siv120b_attributes[] = {
	&dev_attr_write.attr,
	&dev_attr_read.attr,
	NULL
};

static const struct attribute_group siv120b_attr_group = {
	.attrs = siv120b_attributes,
};
...
static int siv120b_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	siv120b.desc.facing = pdata->facing;
	siv120b.desc.orientation = pdata->orientation;
	sysfs_create_group(&client->dev.kobj, &siv120b_attr_group);
