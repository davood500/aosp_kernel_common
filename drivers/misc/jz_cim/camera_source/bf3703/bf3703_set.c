...
{
	  sensor_write_reg(client,0x6b,0x02);
...
{
		sensor_write_reg(client,0x17,0x24);
		sensor_write_reg(client,0x18,0x7c);
		sensor_write_reg(client,0x19,0x18);
		sensor_write_reg(client,0x1a,0x60);
		sensor_write_reg(client,0x0a,0x21);
		sensor_write_reg(client,0x10,0x21);

		sensor_write_reg(client,0x3d,0x59); 
		sensor_write_reg(client,0x6b,0x03); 
...
void bf3703_init_setting(struct i2c_client *client,int mirror)
{
	//SOFE RESET
	//sensor_write_reg16(client,0x12,0x80);
	bf3703_reset();
	msleep(10);	
	sensor_write_reg(client,0x11,0x80);//0x90 set freq
...
*/
	...
	*/
	...
	if(1 == mirror) 
		...
		sensor_write_reg(client,0x1e,0x10); //00,10,20,30
	else if(3 == mirror)
		sensor_write_reg(client,0x1e,0x30); //00,10,20,30	
	else
		sensor_write_reg(client,0x1e,0x20); //00,10,20,30
