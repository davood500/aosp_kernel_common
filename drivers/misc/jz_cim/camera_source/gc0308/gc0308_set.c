...
void gc0308_set_ab_50hz(struct i2c_client *client)
{
	...
	sensor_write_reg(client,0x0f,0x00); 
	sensor_write_reg(client,0xe2,0x00); 
	sensor_write_reg(client,0xe3,0x96); 
	sensor_write_reg(client,0xe4,0x02);
	sensor_write_reg(client,0xe5,0x58); 
	sensor_write_reg(client,0xe6,0x02); 
	sensor_write_reg(client,0xe7,0x58); 
	sensor_write_reg(client,0xe8,0x02);
	sensor_write_reg(client,0xe9,0x58); 
	sensor_write_reg(client,0xea,0x0b); 
	sensor_write_reg(client,0xeb,0xb8);
	dprintk("--------------------------ab_50hz ");
}
...
void gc0308_init_setting(struct i2c_client *client)
{
	...
	sensor_write_reg(client,0x1f , 0x03);	//16							   
	...
	sensor_write_reg(client,0x28 , 0x11);	//add														   
											 
	sensor_write_reg(client,0x26 , 0x03);//	03							   
	...
	sensor_write_reg(client,0xAF , 0xFF); */
		...
		#else
		sensor_write_reg(client,0x14 , 0x12);
		#endif		
	#else
		#ifndef CONFIG_GC0308_VMIRROR
		sensor_write_reg(client,0x14 , 0x11);
		#else
		sensor_write_reg(client,0x14 , 0x13);
		#endif		
	#endif
