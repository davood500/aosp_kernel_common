...
void gc0309_set_nightmode(struct i2c_client *client,int enable)
{
	if (!enable)
	{
	dprintk("---------------------------------gc0309_set_nightmode disable");
	}
	else
	{
	dprintk("-------------------------------gc0309_set_nightmode enable");
	}
}   /* gc0309_NightMode */
...
//	sensor_write_reg(client,0x01,0x0a);  
	...
	dprintk("---------------------------------gc0309_init  4");
}

void gc0309_windows_switch(struct i2c_client *client,int width,int height)
{
