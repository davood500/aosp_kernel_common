...
{
	__gpio_as_output(pdata->gpio_en);                                    /* GPA27 */
...
#if 0
#if (AQUILA_CPU_V1_0 == 1) && (AQUILA_CPU_V1_1 == 0)	/* only for aquila v1.0 */
	__gpio_as_output(GPIO_VIBRATOR_EN); /* lcd rest pin */
	__gpio_set_pin(GPIO_VIBRATOR_EN);
...
#if 0
#if (AQUILA_CPU_V1_0 == 1) && (AQUILA_CPU_V1_1 == 0)	/* only for aquila v1.0 */
	__gpio_clear_pin(GPIO_VIBRATOR_EN); /* lcd rest pin */
#endif
#endif
...
static int ov3640_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ov3640_sensor_desc.facing = pdata->facing;
	ov3640_sensor_desc.orientation = pdata->orientation;
