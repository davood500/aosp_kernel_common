...
//#define GPIO_CAMERA_PDN         (32*4+12) /*GPE13*/
...
{ 
	...
	mdelay(5);
//	dprintk("-----------------------------hi704_power_down\n");
}   

void hi704_power_up(void)
{
	__gpio_as_output(pdata->gpio_en);            /*PE12*/
	__gpio_clear_pin(pdata->gpio_en);
	mdelay(5);
//	dprintk("-----------------------------hi704_power_up\n");
}
...
int hi704_sensor_probe(void)
{
	...
	printk("---------------------------hi704 sensor read is %d",sensor_id);
	...
	printk("---------------------------sensor read is %d",sensor_id);
	return -1;
}
...
static int hi704_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	hi704.desc.facing = pdata->facing;
	hi704.desc.orientation = pdata->orientation;
