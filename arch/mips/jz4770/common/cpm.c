...
unsigned int cpm_get_pllout(void)
{
	...
	nf = ((pll_ctrl & CPPCR0_PLLM_MASK) >> CPPCR0_PLLM_LSB) + 1;
	nr = ((pll_ctrl & CPPCR0_PLLN_MASK) >> CPPCR0_PLLN_LSB) + 1;
	od = (pll_ctrl & CPPCR0_PLLOD_MASK) >> CPPCR0_PLLOD_LSB;
	no = 1 << od;
	pllout = (exclk / (nr * no)) * nf;
...
unsigned int cpm_get_pllout1(void)
{
	...
	unsigned int nf, nr, od, no, val, div;
	...
	nf = ((pll_ctrl & CPPCR1_PLL1M_MASK) >> CPPCR1_PLL1M_LSB) + 1;
	nr = ((pll_ctrl & CPPCR1_PLL1N_MASK) >> CPPCR1_PLL1N_LSB) + 1;
	od = (pll_ctrl & CPPCR1_PLL1OD_MASK) >> CPPCR1_PLL1OD_LSB;
	no = 1 << od;
	pll_out = (clock_in / (nr * no)) * nf;
...
void cpm_start_clock(clock_gate_module module_name)
{
	...
	switch (cgr_index) {
		...
		case 1:
			...
			break;
		default:
			printk("\n\n\nWARNING: can NOT start the %d's clock\n\n\n",
					module_name);
			break;
	}
}
...
void cpm_stop_clock(clock_gate_module module_name)
{
	...
	switch (cgr_index) {
		...
		default:
			printk("\n\n\nWARNING: can NOT stop the %d's clock\n\n\n",
					module_name);
			break;
	}
}
...
unsigned int cpm_set_clock(cgu_clock clock_name, unsigned int clock_hz)
{
	...
	switch (clock_name) {
		...
		case CGU_MSC1CLK:
			...
			OUTREG32(CPM_MSC1CDR, MSCCDR_MPCS | div);
			break;
		...
		case CGU_MSC2CLK:
			...
			OUTREG32(CPM_MSC2CDR, MSCCDR_MPCS | div);
			break;

		case CGU_TVECLK:
			pllclk1 = cpm_get_pllout1();
		...
		case CGU_LPCLK:
			...
			/* if(div == 28) // pll0 */
			/* 	div+=1; */
			if ( pllclk1 > 800000000 ) {
				if(div == 0x14)	{	/* 2011-09-09, new chip maybe lcd-pclk error if PLL1=888M, GPU=444M, LCD=42M. */
					div+=1;
					//printk("pllclk1 > 800000000 div == 0x14\n" );
				}
			}
			#endif
			div = __check_div(div, LPCDR_PIXDIV_LSB, LPCDR_PIXDIV_MASK);
			/* Select pll0 clock as input */
			OUTREG32(CPM_LPCDR, LPCDR_LPCS | div);
			break;
...
{
	unsigned int clock;
#define PSWCLOCK1 100
#define PSWCLOCK2 40
	unsigned int ns4;
	unsigned int clock_ns4;
	clock = cpm_get_clock(CGU_PCLK);	
	ns4 = 1000 * 1000 * 1000*4LL;
	clock_ns4 = ns4 / clock;

	REG_CPM_PSWC1ST = (PSWCLOCK1 * 4 + clock_ns4 - 1) / clock_ns4 + 3;
	REG_CPM_PSWC2ST = (PSWCLOCK2 * 4 + clock_ns4 - 1) / clock_ns4 + 3;
	REG_CPM_LCR &= ~(0xfff << 8);

	REG_CPM_LCR |= (0 << 8); //may be is good

	printk("REG_CPM_PSWC1ST = 0x%08x\n",REG_CPM_PSWC1ST);
	printk("REG_CPM_PSWC2ST = 0x%08x\n",REG_CPM_PSWC2ST);

	printk("REG_CPM_PSWC0ST = 0x%08x\n",REG_CPM_PSWC0ST);
	printk("REG_CPM_PSWC3ST = 0x%08x\n",REG_CPM_PSWC3ST);

}
void cpm_power_ctrl(unsigned int devid,int poweron)   //devid 0 - ahb1 1 - gps 
{
	int lcr_crbit = 0;
	int lcr_stbit = 0;
	...
	if(!strcmp(jz4770_cpu_version,JZ4770_V1)) {
		switch(devid)
		{
#ifdef CPM_POWER_AHB1
			case CPM_POWER_AHB1:
				lcr_crbit = LCR_PDAHB1;
				lcr_stbit = LCR_PDAHB1S;
				break;		
#endif
#ifdef CPM_POWER_GPS
			case CPM_POWER_GPS:
				lcr_crbit = LCR_PDGPS;
				lcr_stbit = LCR_PDGPSS;
				break;
#endif
			default:
				printk("cpm_power_ctrl not find the id(%d)\n",devid);

		}
		local_irq_save(cpuflags);
		if(poweron)
		{
			CLRREG32(CPM_LCR,lcr_crbit);		
		}else
		{
			SETREG32(CPM_LCR, lcr_crbit);
		}
