...
#if defined(CONFIG_NAND_DEVICE_ID) 
#include "mach/board_device_id.h"

/*
 * Tablet ID
 * Return device_id, in mtdblock x
 */
static int device_id_store(struct file *file, const char *buffer, unsigned long count, void *data)
{
	if ( ingenic_board_id == NULL) {
		ingenic_board_id = kmalloc(512,GFP_KERNEL);
		if ( ingenic_board_id == NULL) {
			return 0;
		}
		memset(ingenic_board_id,0,512);
		memcpy(ingenic_board_id,buffer,count);
		return count;
	}
	return 0;
}

static int device_id_read_proc(char *page, char **start, off_t off,
			  int count, int *eof, void *data)
{
	int len = 0;
	int ret;
	...
	if ( ingenic_board_id == NULL) {
		ret = board_read_device_id();
		//printk("board_read_device_id ret=%x\n", ret);

		if (ret !=0 ) {
			printk("board_read_device_id err. ret=%x\n", ret);
			return 0;
		}
	}
	...
	len += sprintf (page+len, ingenic_board_id);
	len += sprintf (page+len, "\n");

	return len;
}
#endif	/* defined(CONFIG_JZ4760_PT701) || defined(CONFIG_JZ4760_PT702) || defined(CONFIG_JZ4760_TABLET_P2)*/
...
static int __init jz_proc_init(void)
{
	...
	/* device id of tablet */
	res = create_proc_entry("device_id", 0644, proc_jz_root);
	if (res) {
		res->read_proc = device_id_read_proc;
		res->write_proc = device_id_store;
		res->data = NULL;
	}
#endif
