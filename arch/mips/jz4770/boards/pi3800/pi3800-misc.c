/*
 * linux/arch/mips/jz4760/board-pi3800.c
 *
...
/* MSC SETUP */
static void pi3800_inand_gpio_init(struct device *dev)
{
	__gpio_as_msc0_pa_4bit();
}

static void pi3800_inand_power_on(struct device *dev)
{
}

static void pi3800_inand_power_off(struct device *dev)
{
}

static void pi3800_inand_cpm_start(struct device *dev)
{
	cpm_set_clock(CGU_MSC0CLK, 25 * 1000 * 1000);
	cpm_start_clock(CGM_MSC0);
}

static unsigned int pi3800_inand_status(struct device *dev)
{
	unsigned int status;
...
struct mmc_partition_info pi3800_partitions[] = {
...
static struct jz_mmc_platform_data pi3800_inand_data = {
	...
	.init           = pi3800_inand_gpio_init,
	.power_on       = pi3800_inand_power_on,
	.power_off      = pi3800_inand_power_off,
	.cpm_start      = pi3800_inand_cpm_start,
	.status		= pi3800_inand_status,
...
//	.max_bus_width  = MMC_CAP_4_BIT_DATA,
	...
	.partitions = pi3800_partitions,
	.num_partitions = ARRAY_SIZE(pi3800_partitions),
};

static void pi3800_tf_gpio_init(struct device *dev)
{
...
static void pi3800_tf_power_on(struct device *dev)
{
	__gpio_clear_pin(GPIO_SD2_VCC_EN_N);
}

static void pi3800_tf_power_off(struct device *dev)
{
	__gpio_set_pin(GPIO_SD2_VCC_EN_N);
}

static void pi3800_tf_cpm_start(struct device *dev)
{
	cpm_set_clock(CGU_MSC2CLK, 25 * 1000 * 1000);
	cpm_start_clock(CGM_MSC2);
}

static unsigned int pi3800_tf_status(struct device *dev)
{
	unsigned int status;
...
static void pi3800_tf_plug_change(int state)
{
...
static struct jz_mmc_platform_data pi3800_tf_data = {
...
#endif
	.ocr_mask	= MMC_VDD_32_33 | MMC_VDD_33_34,
	.init           = pi3800_tf_gpio_init,
	.status_irq	= MSC2_HOTPLUG_IRQ,
	.detect_pin     = GPIO_SD2_CD_N,
	.power_on       = pi3800_tf_power_on,
	.power_off      = pi3800_tf_power_off,
	.cpm_start      = pi3800_tf_cpm_start,
	.status		= pi3800_tf_status,
	.plug_change	= pi3800_tf_plug_change,
...
static void pi3800_sdio_gpio_init(struct device *dev)
{
	//__gpio_as_msc2_pe_4bit();
	__gpio_as_msc1_pe_4bit();
}

static void pi3800_sdio_power_on(struct device *dev)
{
}

static void pi3800_sdio_power_off(struct device *dev)
{
}

static void pi3800_sdio_cpm_start(struct device *dev)
{
	cpm_set_clock(CGU_MSC1CLK, 25 * 1000 * 1000);
	cpm_start_clock(CGM_MSC1);
}

static unsigned int pi3800_sdio_status(struct device *dev)
{
	unsigned int status;
...
static struct jz_mmc_platform_data pi3800_sdio_data = {
...
#endif
	.ocr_mask	= MMC_VDD_32_33 | MMC_VDD_33_34,
	.init           = pi3800_sdio_gpio_init,
	.power_on       = pi3800_sdio_power_on,
	.power_off      = pi3800_sdio_power_off,
	.cpm_start      = pi3800_sdio_cpm_start,
	.status		= pi3800_sdio_status,
	.max_bus_width  = MMC_CAP_4_BIT_DATA, 
...
void __init board_msc_init(void)
{
	...
	jz_add_msc_devices(0, &pi3800_inand_data);
#endif
#ifdef CONFIG_MSC1_JZ4770
	jz_add_msc_devices(1, &pi3800_sdio_data);
#endif
#ifdef CONFIG_MSC2_JZ4770
	jz_add_msc_devices(2, &pi3800_tf_data);
#endif
}
...
void IW8101_wlan_power_off(int flag)
{
	...
	case RESET:
		...
		__gpio_as_output(GPIO_WLAN_PW_EN);
		__gpio_clear_pin(GPIO_WLAN_PW_EN);
		break;
...
void IW8101_wlan_power_on(int flag)
{
	...
	// Turn on power supply
		...
		__gpio_as_output(GPIO_WLAN_PW_EN);
		__gpio_set_pin(GPIO_WLAN_PW_EN);
...
{
	printk("JZ4770 pi3800 board setup\n");
//	jz_restart(NULL);
	board_cpm_setup();
	board_gpio_setup();
	board_wlan_setup();
...
static struct i2c_board_info pi3800_i2c0_devs[] __initdata = {
#ifdef CONFIG_TOUCHSCREEN_CTP_IT7260
	{
...
static struct i2c_board_info pi3800_i2c1_devs[] __initdata = {
...
static struct i2c_board_info pi3800_i2c2_devs[] __initdata = {
	{
	},
};
#ifdef CONFIG_GC0307
static struct jz_cim_sensor_platform_data gc0307_platform_pdata = {
...
#endif
#ifdef CONFIG_MSD0838
static struct jz_cim_sensor_platform_data msd0838_platform_pdata = {
	.facing = CAMERA_FACING_FRONT,
  	.orientation = 0,
	.gpio_rst = GPIO_CAMERA_RST,
  	.gpio_en = GPIO_CAMERA_PDN,
...
static struct i2c_board_info pi3800_i2c3_devs[] __initdata = {
...
static struct i2c_board_info pi3800_i2c4_devs[] __initdata = {
#ifdef CONFIG_GC0307	
//camera
	{
		I2C_BOARD_INFO("gc0307", 0x21),
		.platform_data = &gc0307_platform_pdata,
	},
#endif
#ifdef CONFIG_GC0308
    {
...
#ifdef CONFIG_MSD0838
    {
            I2C_BOARD_INFO("msd0838", 0x18),
            .platform_data = &msd0838_platform_pdata,
    },
#endif
...
static struct battery_info jz47xx_battery_info = {
	.max_vol = 4100000,
	.min_vol = 3650000,
	.dc_chg_max_vol = 4100000,
	.dc_chg_min_vol = 3800000,
	.usb_chg_max_vol = 4100000,
	.usb_chg_min_vol = 3700000,
	.battery_mah = 2000,
	.dc_charg_ma = 800,
	.usb_charg_ma = 600,
};
...
struct hdmi_it6610_board_info pi3800_ite6610_board_info = {
	.pin_info = &pi3800_it6610_pin_info,
#ifdef CONFIG_HDMI_HOTPLUG_HPD_CONNECT_LOW_ACTIVE
	.hpd_connect_active = 0,
#else
	.hpd_connect_active = 1,
#endif
	.hdmi_board_init = pi3800_hdmi_board_init,
	.hdmi_power_on = pi3800_hdmi_power_on,
	.hdmi_power_off = pi3800_hdmi_power_off,
};

static struct platform_device board_ite6610_device = {
        .name = "ite6610-drv",
		.dev = { .platform_data = &pi3800_ite6610_board_info },
};
#endif
...
static struct gpio_led jz_charge_leds[] = {
	{
		.name = "charge_full",
		.gpio = GPIO_CHARG_LED,
		.active_low = 1,
...
static struct gpio_led_platform_data jz_charge_led_pdata = {
	.num_leds	=	ARRAY_SIZE(jz_charge_leds),
	.leds		=	jz_charge_leds,
};

static struct platform_device jz_charge_led_device = {
	.name	=	 "leds-gpio",
	.id		=	-1,
	.dev 	= 	{ .platform_data = &jz_charge_led_pdata },
};
#endif //CONFIG_LEDS_GPIO

static int __init pi3800_board_init(void)
{
	i2c_register_board_info(0, pi3800_i2c0_devs, ARRAY_SIZE(pi3800_i2c0_devs));
	i2c_register_board_info(1, pi3800_i2c1_devs, ARRAY_SIZE(pi3800_i2c1_devs));
	i2c_register_board_info(2, pi3800_i2c2_devs, ARRAY_SIZE(pi3800_i2c2_devs));
#if defined(CONFIG_I2C_GPIO)
	platform_device_register(&i2c3_gpio_device);
	platform_device_register(&i2c4_gpio_device);
	i2c_register_board_info(3, pi3800_i2c3_devs, ARRAY_SIZE(pi3800_i2c3_devs));
	i2c_register_board_info(4, pi3800_i2c4_devs, ARRAY_SIZE(pi3800_i2c4_devs));
#endif
#if defined(CONFIG_JZ_CIM)
...
#if defined(CONFIG_LEDS_GPIO)	
	platform_device_register(&jz_charge_led_device);
#endif

	return 0;
}

arch_initcall(pi3800_board_init);
