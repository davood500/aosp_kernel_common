...
static int linden_dlv_set_device(struct snd_device_config *snd_dev_cfg)
{
	int ret;
...
static int linden_dlv_set_gpio_before_set_route(int route)
{
	DUMP_FUNC();
	switch(route){
...
static int linden_dlv_set_gpio_after_set_route(int route)
{
	DUMP_FUNC();
...
static int linden_dlv_turn_off_part(int mode)
{
	DUMP_FUNC();
...
static int linden_dlv_shutdown_part(void)
{
	DUMP_FUNC();
...
static int linden_dlv_suspend_part(void)
{
	DUMP_FUNC();
...
static int linden_dlv_resume_part(void)
{
	DUMP_FUNC();
...
static int linden_dlv_anti_pop_part(void)
{
	DUMP_FUNC();
...
static jz_dlv_platform_data_t jz_dlv_platform_data = {
	...
	.dlv_set_device = linden_dlv_set_device,
        .dlv_set_gpio_before_set_route = linden_dlv_set_gpio_before_set_route,
	.dlv_set_gpio_after_set_route = linden_dlv_set_gpio_after_set_route,
	.dlv_turn_off_part = linden_dlv_turn_off_part,
	.dlv_shutdown_part = linden_dlv_shutdown_part,
	.dlv_suspend_part = linden_dlv_suspend_part,
	.dlv_resume_part = linden_dlv_resume_part,
	.dlv_anti_pop_part = linden_dlv_anti_pop_part,
};
...
static int __init linden_dlv_board_init(void)
{
	int ret = 0;
...
arch_initcall(linden_dlv_board_init);
