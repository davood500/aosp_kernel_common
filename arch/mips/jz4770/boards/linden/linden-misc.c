/*
 * linux/arch/mips/jz4760/board-linden.c
 *
...
/* MSC SETUP */
static void linden_inand_gpio_init(struct device *dev)
{
	__gpio_as_msc0_pa_4bit();
}

static void linden_inand_power_on(struct device *dev)
{
}

static void linden_inand_power_off(struct device *dev)
{
}

static void linden_inand_cpm_start(struct device *dev)
{
	cpm_set_clock(CGU_MSC0CLK, 25 * 1000 * 1000);
	cpm_start_clock(CGM_MSC0);
}

static unsigned int linden_inand_status(struct device *dev)
{
	unsigned int status;
...
struct mmc_partition_info linden_partitions[] = {
...
static struct jz_mmc_platform_data linden_inand_data = {
	...
	.init           = linden_inand_gpio_init,
	.power_on       = linden_inand_power_on,
	.power_off      = linden_inand_power_off,
	.cpm_start      = linden_inand_cpm_start,
	.status		= linden_inand_status,
...
//	.max_bus_width  = MMC_CAP_4_BIT_DATA,
	...
	.partitions = linden_partitions,
	.num_partitions = ARRAY_SIZE(linden_partitions),
};

static void linden_tf_gpio_init(struct device *dev)
{
	__gpio_as_msc1_pd_4bit();
	__gpio_as_output(GPIO_SD1_VCC_EN_N);
}

static void linden_tf_power_on(struct device *dev)
{
	__gpio_clear_pin(GPIO_SD1_VCC_EN_N);
}

static void linden_tf_power_off(struct device *dev)
{
	__gpio_set_pin(GPIO_SD1_VCC_EN_N);
}

static void linden_tf_cpm_start(struct device *dev)
{
	cpm_set_clock(CGU_MSC1CLK, 25 * 1000 * 1000);
	cpm_start_clock(CGM_MSC1);
}

static unsigned int linden_tf_status(struct device *dev)
{
	unsigned int status;
...
static void linden_tf_plug_change(int state)
{
...
static struct jz_mmc_platform_data linden_tf_data = {
...
#endif
	.ocr_mask	= MMC_VDD_32_33 | MMC_VDD_33_34,
	.init           = linden_tf_gpio_init,
	.status_irq	= MSC1_HOTPLUG_IRQ,
	.detect_pin     = GPIO_SD1_CD_N,
	.power_on       = linden_tf_power_on,
	.power_off      = linden_tf_power_off,
	.cpm_start      = linden_tf_cpm_start,
	.status		= linden_tf_status,
	.plug_change	= linden_tf_plug_change,
...
static void linden_sdio_gpio_init(struct device *dev)
{
	__gpio_as_msc2_pe_4bit();
}

static void linden_sdio_power_on(struct device *dev)
{
}

static void linden_sdio_power_off(struct device *dev)
{
}

static void linden_sdio_cpm_start(struct device *dev)
{
	cpm_set_clock(CGU_MSC2CLK, 25 * 1000 * 1000);
	cpm_start_clock(CGM_MSC2);
}

static unsigned int linden_sdio_status(struct device *dev)
{
	unsigned int status;
...
static struct jz_mmc_platform_data linden_sdio_data = {
...
#endif
	.ocr_mask	= MMC_VDD_32_33 | MMC_VDD_33_34,
	.init           = linden_sdio_gpio_init,
	.power_on       = linden_sdio_power_on,
	.power_off      = linden_sdio_power_off,
	.cpm_start      = linden_sdio_cpm_start,
	.status		= linden_sdio_status,
	.max_bus_width  = MMC_CAP_4_BIT_DATA, 
...
void __init board_msc_init(void)
{
	...
	jz_add_msc_devices(0, &linden_inand_data);
#endif
#ifdef CONFIG_MSC1_JZ4770
	jz_add_msc_devices(1, &linden_tf_data);
#endif
#ifdef CONFIG_MSC2_JZ4770
	jz_add_msc_devices(2, &linden_sdio_data);
#endif
}
...
{
	printk("JZ4770 linden board setup\n");
//	jz_restart(NULL);
	board_cpm_setup();
	board_gpio_setup();
	board_wlan_setup();
...
static struct i2c_board_info linden_i2c0_devs[] __initdata = {
#ifdef CONFIG_TOUCHSCREEN_CTP_IT7260
	{
...
static struct i2c_board_info linden_i2c1_devs[] __initdata = {
...
static struct i2c_board_info linden_i2c2_devs[] __initdata = {
	{
	},
};
#ifdef CONFIG_GC0307
...
static struct i2c_board_info linden_i2c3_devs[] __initdata = {
...
static struct i2c_board_info linden_i2c4_devs[] __initdata = {
#ifdef CONFIG_GC0307	
//camera
	{
...
struct hdmi_it6610_board_info linden_ite6610_board_info = {
	.pin_info = &linden_it6610_pin_info,
#ifdef CONFIG_HDMI_HOTPLUG_HPD_CONNECT_LOW_ACTIVE
	.hpd_connect_active = 0,
#else
	.hpd_connect_active = 1,
#endif
	.hdmi_board_init = linden_hdmi_board_init,
	.hdmi_power_on = linden_hdmi_power_on,
	.hdmi_power_off = linden_hdmi_power_off,
};

static struct platform_device board_ite6610_device = {
        .name = "ite6610-drv",
		.dev = { .platform_data = &linden_ite6610_board_info },
};
#endif
...
static int __init linden_board_init(void)
{
	i2c_register_board_info(0, linden_i2c0_devs, ARRAY_SIZE(linden_i2c0_devs));
	i2c_register_board_info(1, linden_i2c1_devs, ARRAY_SIZE(linden_i2c1_devs));
	i2c_register_board_info(2, linden_i2c2_devs, ARRAY_SIZE(linden_i2c2_devs));
#if defined(CONFIG_I2C_GPIO)
	platform_device_register(&i2c3_gpio_device);
	platform_device_register(&i2c4_gpio_device);
	i2c_register_board_info(3, linden_i2c3_devs, ARRAY_SIZE(linden_i2c3_devs));
	i2c_register_board_info(4, linden_i2c4_devs, ARRAY_SIZE(linden_i2c4_devs));
#endif
#if defined(CONFIG_JZ_CIM)
...
arch_initcall(linden_board_init);
