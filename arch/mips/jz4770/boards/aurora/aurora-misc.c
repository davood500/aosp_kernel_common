/*
 * linux/arch/mips/jz4770/boards/aurora/aurora-misc.c
 *
...
/* MSC SETUP */
static void aurora_inand_gpio_init(struct device *dev)
{
	__gpio_as_msc0_pa_4bit();
}

static void aurora_inand_power_on(struct device *dev)
{
}

static void aurora_inand_power_off(struct device *dev)
{
}

static void aurora_inand_cpm_start(struct device *dev)
{
	cpm_set_clock(CGU_MSC0CLK, 25 * 1000 * 1000);
	cpm_start_clock(CGM_MSC0);
}

static unsigned int aurora_inand_status(struct device *dev)
{
	unsigned int status;
...
struct mmc_partition_info aurora_partitions[] = {
	...
	[6] = {"system",   28*MBYTE, 256*MBYTE, 1}, 	//28MB - 256MB
	[7] = {"data",    286*MBYTE, 1024*MBYTE, 1}, 	//286MB - 1024MB
	[8] = {"cache",    1315*MBYTE,  128*MBYTE, 1}, 	//1315MB - 128MB
	[9] = {"device_id",1443*MBYTE,   2*MBYTE, 0},	//1443MB - 2MB
	[10]= {"test_0",         0, UINT32_MAX, 0},
};

static struct jz_mmc_platform_data aurora_inand_data = {
	...
	.init           = aurora_inand_gpio_init,
	.power_on       = aurora_inand_power_on,
	.power_off      = aurora_inand_power_off,
	.cpm_start      = aurora_inand_cpm_start,
	.status		= aurora_inand_status,
	...
	.partitions = aurora_partitions,
	.num_partitions = ARRAY_SIZE(aurora_partitions),
};

static void aurora_tf_gpio_init(struct device *dev)
{
...
static void aurora_tf_power_on(struct device *dev)
{
	__gpio_set_pin(GPIO_SD2_VCC_EN_N);
	//__msc2_enable_power();
}

static void aurora_tf_power_off(struct device *dev)
{
//	__gpio_clear_pin(GPIO_SD2_VCC_EN_N);
	//__msc2_disable_power();
}

static void aurora_tf_cpm_start(struct device *dev)
{
	cpm_set_clock(CGU_MSC2CLK, 25 * 1000 * 1000);
	cpm_start_clock(CGM_MSC2);
}

static unsigned int aurora_tf_status(struct device *dev)
{
	unsigned int status;
...
static void aurora_tf_plug_change(int state)
{
...
static struct jz_mmc_platform_data aurora_tf_data = {
...
#endif
	.ocr_mask	= MMC_VDD_32_33 | MMC_VDD_33_34,
	.init           = aurora_tf_gpio_init,
	.status_irq	= MSC2_HOTPLUG_IRQ,
	.detect_pin     = GPIO_SD2_CD_N,
	.power_on       = aurora_tf_power_on,
	.power_off      = aurora_tf_power_off,
	.cpm_start      = aurora_tf_cpm_start,
	.status		= aurora_tf_status,
	.plug_change	= aurora_tf_plug_change,
...
static void aurora_sdio_gpio_init(struct device *dev)
{
	__gpio_as_msc1_pd_4bit();
}

static void aurora_sdio_power_on(struct device *dev)
{
}

static void aurora_sdio_power_off(struct device *dev)
{
}

static void aurora_sdio_cpm_start(struct device *dev)
{
	cpm_set_clock(CGU_MSC1CLK, 25 * 1000 * 1000);
	cpm_start_clock(CGM_MSC1);
}

static unsigned int aurora_sdio_status(struct device *dev)
{
	unsigned int status;
...
static struct jz_mmc_platform_data aurora_sdio_data = {
...
#endif
	.ocr_mask	= MMC_VDD_32_33 | MMC_VDD_33_34,
	.init           = aurora_sdio_gpio_init,
	.power_on       = aurora_sdio_power_on,
	.power_off      = aurora_sdio_power_off,
	.cpm_start      = aurora_sdio_cpm_start,
	.status		= aurora_sdio_status,
...
void __init board_msc_init(void)
{
	...
	jz_add_msc_devices(0, &aurora_inand_data);
#endif

#ifdef CONFIG_MSC1_JZ4770
	jz_add_msc_devices(1, &aurora_sdio_data);
#endif

#ifdef CONFIG_MSC2_JZ4770
	jz_add_msc_devices(2, &aurora_tf_data);
#endif
}
...
static struct i2c_board_info aurora_i2c1_devs[] __initdata = {
#ifdef CONFIG_KEYBOARD_HA2605
	{
...
static struct i2c_board_info aurora_i2c0_devs[] __initdata = {
#ifdef CONFIG_SENSORS_JZ_MMA8452
	{
...
static struct i2c_board_info aurora_i2c2_devs[] __initdata = {
#ifdef CONFIG_TOUCHSCREEN_JZ_GT801
	{
...
static struct i2c_board_info aurora_gpio_i2c3_devs[] __initdata = {
...
static struct i2c_board_info aurora_gpio_i2c4_devs[] __initdata = {
#ifdef CONFIG_GT2005
	{
...
static struct i2c_board_info aurora_gpio_i2c5_devs[] __initdata = {
};
...
struct hdmi_it6610_board_info aurora_ite6610_board_info = {
	.pin_info = &aurora_it6610_pin_info,
#ifdef CONFIG_HDMI_HOTPLUG_HPD_CONNECT_LOW_ACTIVE
	.hpd_connect_active = 0,
#else
	.hpd_connect_active = 1,
#endif
	.hdmi_board_init = aurora_hdmi_board_init,
	.hdmi_power_on = aurora_hdmi_power_on,
	.hdmi_power_off = aurora_hdmi_power_off,
};

static struct platform_device board_ite6610_device = {
	.name = "ite6610-drv",
	.dev = { .platform_data = &aurora_ite6610_board_info },
};
#endif
...
/* ######################################################################### */
static int __init aurora_board_init(void)
{

#if defined(CONFIG_I2C_GPIO)
	i2c_register_board_info(3, aurora_gpio_i2c3_devs, ARRAY_SIZE(aurora_gpio_i2c3_devs));
	i2c_register_board_info(4, aurora_gpio_i2c4_devs, ARRAY_SIZE(aurora_gpio_i2c4_devs));
	i2c_register_board_info(5, aurora_gpio_i2c5_devs, ARRAY_SIZE(aurora_gpio_i2c5_devs));	
	...
	i2c_register_board_info(0, aurora_i2c0_devs, ARRAY_SIZE(aurora_i2c0_devs));
	i2c_register_board_info(1, aurora_i2c1_devs, ARRAY_SIZE(aurora_i2c1_devs));
	i2c_register_board_info(2, aurora_i2c2_devs, ARRAY_SIZE(aurora_i2c2_devs));
...
arch_initcall(aurora_board_init);
