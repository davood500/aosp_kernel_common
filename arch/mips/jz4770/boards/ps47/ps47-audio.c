...
#define RECORD_DIGITAL_VOLUME_BASE      +5 /*val:  0 ~ +43 (dB)*/
...
static int ps47_dlv_set_device(struct snd_device_config *snd_dev_cfg)
{
	int ret;
	...
	case SND_DEVICE_HEADSET_AND_SPEAKER:
		ret = dlv_set_route(REPLAY_HP_STEREO_CAP_LESS_AND_LINEOUT);
		if(ret != REPLAY_HP_STEREO_CAP_LESS_AND_LINEOUT)
		{
...
static int ps47_dlv_set_gpio_before_set_route(int route)
{
	DUMP_FUNC();
	...
	case ROUTE_ALL_CLEAR:
		break;
	case REPLAY_HP_STEREO_CAP_LESS:
	case REPLAY_LINEOUT:	
	case REPLAY_HP_STEREO_CAP_LESS_AND_LINEOUT:
		/*hp mute*/
...
static int ps47_dlv_set_gpio_after_set_route(int route)
{
	DUMP_FUNC();
...
static int ps47_dlv_turn_off_part(int mode)
{
	DUMP_FUNC();
...
static int ps47_dlv_shutdown_part(void)
{
	DUMP_FUNC();
...
static int ps47_dlv_suspend_part(void)
{
	DUMP_FUNC();
...
static int ps47_dlv_resume_part(void)
{
	DUMP_FUNC();
...
static int ps47_dlv_anti_pop_part(void)
{
	DUMP_FUNC();
...
static jz_dlv_platform_data_t jz_dlv_platform_data = {
	...
	.default_replay_route = REPLAY_LINEOUT,
	.default_record_route = RECORD_MIC1_MONO_DIFF_WITH_BIAS,

	.dlv_set_device = ps47_dlv_set_device,
        .dlv_set_gpio_before_set_route = ps47_dlv_set_gpio_before_set_route,
	.dlv_set_gpio_after_set_route = ps47_dlv_set_gpio_after_set_route,
	.dlv_turn_off_part = ps47_dlv_turn_off_part,
	.dlv_shutdown_part = ps47_dlv_shutdown_part,
	.dlv_suspend_part = ps47_dlv_suspend_part,
	.dlv_resume_part = ps47_dlv_resume_part,
	.dlv_anti_pop_part = ps47_dlv_anti_pop_part,
};
...
static int __init ps47_dlv_board_init(void)
{
	int ret = 0;
...
arch_initcall(ps47_dlv_board_init);
