/*
 * linux/arch/mips/jz4760/board-npm703.c
 *
...
/* MSC SETUP */
static void npm703_inand_gpio_init(struct device *dev)
{
	__gpio_as_msc0_pa_4bit();
}

static void npm703_inand_power_on(struct device *dev)
{
}

static void npm703_inand_power_off(struct device *dev)
{
}

static void npm703_inand_cpm_start(struct device *dev)
{
	cpm_set_clock(CGU_MSC0CLK, 25 * 1000 * 1000);
	cpm_start_clock(CGM_MSC0);
}

static unsigned int npm703_inand_status(struct device *dev)
{
	unsigned int status;
...
struct mmc_partition_info npm703_partitions[] = {
...
static struct jz_mmc_platform_data npm703_inand_data = {
	...
	.init           = npm703_inand_gpio_init,
	.power_on       = npm703_inand_power_on,
	.power_off      = npm703_inand_power_off,
	.cpm_start      = npm703_inand_cpm_start,
	.status		= npm703_inand_status,
	...
	.partitions = npm703_partitions,
	.num_partitions = ARRAY_SIZE(npm703_partitions),
};

static void npm703_tf_gpio_init(struct device *dev)
{
...
static void npm703_tf_power_on(struct device *dev)
{
	__gpio_set_pin(GPIO_SD2_VCC_EN_N);
	//__msc2_enable_power();
}

static void npm703_tf_power_off(struct device *dev)
{
//	__gpio_clear_pin(GPIO_SD2_VCC_EN_N);
	//__msc2_disable_power();
}

static void npm703_tf_cpm_start(struct device *dev)
{
	cpm_set_clock(CGU_MSC2CLK, 25 * 1000 * 1000);
	cpm_start_clock(CGM_MSC2);
}

static unsigned int npm703_tf_status(struct device *dev)
{
	unsigned int status;
...
static void npm703_tf_plug_change(int state)
{
...
static struct jz_mmc_platform_data npm703_tf_data = {
...
#endif
	.ocr_mask	= MMC_VDD_32_33 | MMC_VDD_33_34,
	.init           = npm703_tf_gpio_init,
	.status_irq	= MSC2_HOTPLUG_IRQ,
	.detect_pin     = GPIO_SD2_CD_N,
	.power_on       = npm703_tf_power_on,
	.power_off      = npm703_tf_power_off,
	.cpm_start      = npm703_tf_cpm_start,
	.status		= npm703_tf_status,
	.plug_change	= npm703_tf_plug_change,
...
static void npm703_sdio_gpio_init(struct device *dev)
{
	__gpio_as_msc1_pd_4bit();
}

static void npm703_sdio_power_on(struct device *dev)
{
}

static void npm703_sdio_power_off(struct device *dev)
{
}

static void npm703_sdio_cpm_start(struct device *dev)
{
	cpm_set_clock(CGU_MSC1CLK, 25 * 1000 * 1000);
	cpm_start_clock(CGM_MSC1);
}

static unsigned int npm703_sdio_status(struct device *dev)
{
	unsigned int status;
...
static struct jz_mmc_platform_data npm703_sdio_data = {
...
#endif
	.ocr_mask	= MMC_VDD_32_33 | MMC_VDD_33_34,
	.init           = npm703_sdio_gpio_init,
	.power_on       = npm703_sdio_power_on,
	.power_off      = npm703_sdio_power_off,
	.cpm_start      = npm703_sdio_cpm_start,
	.status		= npm703_sdio_status,
...
void __init board_msc_init(void)
{
	...
	jz_add_msc_devices(0, &npm703_inand_data);
#endif

#ifdef CONFIG_MSC1_JZ4770
	jz_add_msc_devices(1, &npm703_sdio_data);
#endif

#ifdef CONFIG_MSC2_JZ4770
	jz_add_msc_devices(2, &npm703_tf_data);
#endif
}
...
static struct i2c_board_info npm703_i2c1_devs[] __initdata = {
#ifdef CONFIG_KEYBOARD_HA2605
	{
...
static struct i2c_board_info npm703_i2c0_devs[] __initdata = {
#ifdef CONFIG_SENSORS_JZ_MMA8452
	{
...
static struct i2c_board_info npm703_i2c2_devs[] __initdata = {
#ifdef CONFIG_TOUCHSCREEN_JZ_GT801
	{
...
static struct i2c_board_info npm703_gpio_i2c3_devs[] __initdata = {
...
static struct i2c_board_info npm703_gpio_i2c4_devs[] __initdata = {
#ifdef CONFIG_GT2005
	{
...
static struct i2c_board_info npm703_gpio_i2c5_devs[] __initdata = {
};
...
struct hdmi_it6610_board_info npm703_ite6610_board_info = {
	.pin_info = &npm703_it6610_pin_info,
#ifdef CONFIG_HDMI_HOTPLUG_HPD_CONNECT_LOW_ACTIVE
	.hpd_connect_active = 0,
#else
	.hpd_connect_active = 1,
#endif
	.hdmi_board_init = npm703_hdmi_board_init,
	.hdmi_power_on = npm703_hdmi_power_on,
	.hdmi_power_off = npm703_hdmi_power_off,
};

static struct platform_device board_ite6610_device = {
	.name = "ite6610-drv",
	.dev = { .platform_data = &npm703_ite6610_board_info },
};
#endif
...
/* ######################################################################### */
static int __init npm703_board_init(void)
{

#if defined(CONFIG_I2C_GPIO)
	i2c_register_board_info(3, npm703_gpio_i2c3_devs, ARRAY_SIZE(npm703_gpio_i2c3_devs));
	i2c_register_board_info(4, npm703_gpio_i2c4_devs, ARRAY_SIZE(npm703_gpio_i2c4_devs));
	i2c_register_board_info(5, npm703_gpio_i2c5_devs, ARRAY_SIZE(npm703_gpio_i2c5_devs));	
	...
	i2c_register_board_info(0, npm703_i2c0_devs, ARRAY_SIZE(npm703_i2c0_devs));
	i2c_register_board_info(1, npm703_i2c1_devs, ARRAY_SIZE(npm703_i2c1_devs));
	i2c_register_board_info(2, npm703_i2c2_devs, ARRAY_SIZE(npm703_i2c2_devs));
...
arch_initcall(npm703_board_init);
