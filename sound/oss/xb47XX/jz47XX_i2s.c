...
{
	int ret = init_audio_node(&endpoint->mem, pagesize, count,(int *)&endpoint->fragmem_start);
	if (ret) {
 ...
 */
extern void dlv_write_reg(int addr, int val);
...
#endif
#if 0
	/* do set codec register when count > 1 */
	if (count > 8) {
		/* it should be a 0xXX */
		copy_from_user(reg_char, buffer + 2, 2);
		sscanf(reg_char, "%2x", &reg_sel);
		copy_from_user(reg_char, buffer + 7, 2);
		sscanf(reg_char, "%2x", &reg_val);
		printk("input %s %2x %2x \n",buffer, reg_sel, reg_val);
		dlv_write_reg(reg_sel, reg_val);
		return count;
	}
#endif
...
do{				\
	printk("audio external i2s codec gpio don't configure!!!\n");\
}while(0)
#endif
...
{
	if(g_jz_i2s_codec == JZ_EXTERNAL_CODEC)
		return ;
	g_jz_i2s_codec = JZ_EXTERNAL_CODEC;
...
{
	if(g_jz_i2s_codec == JZ_INTERNAL_CODEC)
		return ;
...
//static int jz_i2s_ioctl_mixdev(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SND_SELECT_CODEC:
	{
		...
		put_user(g_jz_i2s_codec, (long *) arg);
		break;
	}
#if 0
...
*/
	...
	if(__gpio_get_pin(GPIO_HEAD_DET)){
		__gpio_as_irq_high_level(GPIO_HEAD_DET);
	}else{
		__gpio_as_irq_low_level(GPIO_HEAD_DET);
	}	
#endif
...
void i2s_controller_init(void)
{
	...
	__i2s_send_rfirst();            //send right channel first
